...
const PublicationPreviewPage: React.FC<PublicationPreviewPageProps> = ({
  ...
  const handleChatContact = async () => {
    if (!api.isAuthenticated()) {
      toast.info("Veuillez vous connecter pour envoyer un message");
      return;
    }
    ...
    try {
      ...
      if (error?.response?.data?.chat_id) {
        onTabChange("messages");
        sessionStorage.setItem("selectedChatId", error.response.data.chat_id.toString());
      } else {
          ...
          <div style={{ padding: "24px" }}>
            ...
            >
              ...
              <button
                ...
                onMouseLeave={(e) => {
                  e.currentTarget.style.backgroundColor = MAIN_BLUE;
                }}
              >
                <img
                  src={commentIcon}
